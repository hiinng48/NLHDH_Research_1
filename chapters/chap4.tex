...
\State \textbf{Shared (barrier 4):} \texttt{bMutex} $\leftarrow 1$; \texttt{bCount} $\leftarrow 0$; \texttt{bSem} $\leftarrow 0$
...
\Function{Barrier4}{}
    ...
    \If{$\texttt{bCount} = 4$}
        \For{$i \gets 1$ \textbf{to} $4$}
            \State \Call{signal}{\texttt{bSem}}
        \EndFor
        \State $\texttt{bCount} \gets 0$
    \EndIf
    \State \Call{signal}{\texttt{bMutex}}
    \State \Call{wait}{\texttt{bSem}}
\EndFunction
