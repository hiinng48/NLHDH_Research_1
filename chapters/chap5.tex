...
\subsubsection*{Mã giả cho hành khách}
...
Direction waitingDirectionAt(int f) {
    if (!waitQ[f][1].empty()) {
        return UP;
    }
    if (!waitQ[f][0].empty()) {
...
bool shouldStopHere() {
    if (!exitQ[currentFloor].empty()) {
        return true;
    }
    if (load == CAPACITY) {
        // Thang day: chi dung de tra khach, khong dung chi de don them.
        return false;
    }
    if (direction == IDLE) {
        return hasWaitingAt(currentFloor);
    }
    return !waitQ[currentFloor][dirIndex(direction)].empty();
}
...
void updateDirection() {
    if (direction == IDLE) {
        Direction here = waitingDirectionAt(currentFloor);

        if (here != IDLE) {
            direction = here;
        }
        else if (hasRequestAbove(currentFloor)) {
            direction = UP;
        }
...
Hàm \texttt{shouldStopHere()} luôn ưu tiên trả khách ở tầng hiện tại. Nếu
thang đã đầy, thang không dừng chỉ vì có người chờ lên ở tầng đó; nó tiếp tục
di chuyển đến tầng có khách cần ra để tránh bị kẹt tại một tầng không thể đón
thêm người.

\subsubsection*{Mã giả cho thang máy}
...
Bài toán Thang máy minh họa đồng bộ trong một dịch vụ có nhiều hàng đợi,
nhiều điều kiện chờ và giới hạn tài nguyên rõ ràng. Lời giải dùng mutex để
bảo vệ trạng thái thang máy, semaphore riêng cho từng hành khách để tránh
đánh thức nhầm, và chính sách SCAN/LOOK để phục vụ yêu cầu theo hướng ổn
định. Nhờ đặt chỗ trước khi báo cho hành khách vào thang, hệ thống giữ được
bất biến \texttt{load <= CAPACITY} và hạn chế starvation.
...
\subsubsection*{Giải thích cơ chế chuyển quyền}
